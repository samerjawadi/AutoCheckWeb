    ...
    <div className="page-enter p-3 md:p-6 w-full">
      ...
      <div className="md:hidden flex flex-col gap-3 mb-4">
        {loading ? (
          ...
          <div key={s.id} className="bg-neutral-900 border border-neutral-800 rounded-xl p-4 flex flex-col gap-3">
            <div className="flex items-start justify-between gap-3">
              <div>
                <p className="text-neutral-100 font-semibold leading-tight">{s.name}</p>
                <span className={`inline-block mt-1 text-xs px-2 py-0.5 rounded-full font-medium ${TYPE_STYLE[s.type] ?? TYPE_STYLE.Other}`}>
              ...
              <div className="flex items-center gap-1">
                ...
                <button onClick={() => openEdit(s)} className="p-2 text-neutral-400 hover:text-yellow-400 hover:bg-neutral-700 rounded transition-colors cursor-pointer" title={t("edit")}>
                  <HiPencil className="w-4 h-4" />
                </button>
                <button onClick={() => openDelete(s)} className="p-2 text-neutral-400 hover:text-red-400 hover:bg-neutral-700 rounded transition-colors cursor-pointer" title={t("delete")}>
            ...
            <div className="flex flex-wrap gap-1.5">
              {strToPhones(s.phone).filter(Boolean).length === 0 ? (
                ...
                <span key={i} className="text-xs bg-neutral-800 px-2 py-1 rounded-full text-neutral-400">{p}</span>
              ))}
            </div>
