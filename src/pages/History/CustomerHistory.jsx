...
export default function CustomerHistory() {
  ...
  useEffect(() => {
    const load = async () => {
      const [customer, allJobs, allCars] = await Promise.all([
        db.customers.getById(customerId),
        db.jobs.getAll(),
        db.cars.getAll(),
      ]);

      const jobs = allJobs.filter((j) => j.customerId === customerId);
      const cars = allCars.filter((c) => c.customerId === customerId);

      // Pre-attach payLabel for filtering
      const enriched = jobs.map((j) => ({ ...j, _payLabel: payLabel(j.lines, j.payments) }));

      const totalSpent  = jobs.reduce((s, j) => s + calcPaid(j.payments), 0);
  ...
  return (
    <div className="page-enter p-3 md:p-6 w-full">
      ...
      <button onClick={() => navigate(-1)}
        ...
        <HiArrowLeft className="w-4 h-4" /> {t("nav_customers")}
      </button>

      {/* Header */}
      <div className="flex flex-wrap items-start justify-between gap-4 mb-6">
        <div>
          <h1 className="text-2xl font-bold text-neutral-100">{customer?.name ?? "—"}</h1>
          <p className="text-sm text-neutral-500 mt-0.5">{customer?.phone}{customer?.email ? ` · ${customer.email}` : ""}</p>
        </div>
        {/* Summary cards */}
        <div className="flex gap-3 flex-wrap">
          ...
          <div className="bg-neutral-900 border border-neutral-800 rounded-xl px-5 py-3 text-center">
            ...
            <p className="text-xs text-neutral-500 mt-0.5">{t("dashboard_collected")}</p>
          </div>
          <div className={`bg-neutral-900 border border-neutral-800 rounded-xl px-5 py-3 text-center`}>
            <p className={`text-xl font-bold font-mono ${totalBalance > 0 ? "text-orange-400" : "text-green-400"}`}>{fmt(Math.max(0, totalBalance))}</p>
            <p className="text-xs text-neutral-500 mt-0.5">{t("jobs_balance_due")}</p>
          </div>
        </div>
      </div>
      ...
      {cars.length > 0 && (
        <div className="mb-6">
          <h2 className="text-xs font-semibold text-neutral-500 uppercase tracking-widest mb-3">{t("nav_cars")}</h2>
      ...
      {/* Job history */}
      <h2 className="text-xs font-semibold text-neutral-500 uppercase tracking-widest mb-3">{t("nav_jobs")} — {t("period_all")}</h2>
