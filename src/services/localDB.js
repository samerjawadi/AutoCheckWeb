...
export const db = {
  customers: {
    getAll:   async ()       => { const { data } = await supabase.from("customers").select("*").order("name"); return (data ?? []).map(toCustomer); },
    getById:  async (id)     => { const { data } = await supabase.from("customers").select("*").eq("id", id).single(); return data ? toCustomer(data) : null; },
    add:      async (item)   => { const { data, error } = await supabase.from("customers").insert({ name: item.name, phone: item.phone, email: item.email ?? "" }).select().single(); if (error) { console.error("customers.add error:", error); throw error; } return toCustomer(data); },
    update:   async (id, p)  => { const d = {}; if (p.name !== undefined) d.name = p.name; if (p.phone !== undefined) d.phone = p.phone; if (p.email !== undefined) d.email = p.email; await supabase.from("customers").update(d).eq("id", id); },
    delete:   async (id)     => { await supabase.from("customers").delete().eq("id", id); },
  },

  cars: {
    getAll:   async ()       => { const { data } = await supabase.from("cars").select("*").order("plate"); return (data ?? []).map(toCar); },
    getById:  async (id)     => { const { data } = await supabase.from("cars").select("*").eq("id", id).single(); return data ? toCar(data) : null; },
    add:      async (item)   => { const { data } = await supabase.from("cars").insert({ customer_id: item.customerId, plate: item.plate, vin: item.vin ?? "", manufacturer: item.manufacturer, model: item.model, description: item.description ?? "" }).select().single(); return toCar(data); },
    update:   async (id, p)  => { const d = {}; if (p.customerId !== undefined) d.customer_id = p.customerId; if (p.plate !== undefined) d.plate = p.plate; if (p.vin !== undefined) d.vin = p.vin; if (p.manufacturer !== undefined) d.manufacturer = p.manufacturer; if (p.model !== undefined) d.model = p.model; if (p.description !== undefined) d.description = p.description; await supabase.from("cars").update(d).eq("id", id); },
    delete:   async (id)     => { await supabase.from("cars").delete().eq("id", id); },
  },

  suppliers: {
    getAll:   async ()       => { const { data } = await supabase.from("suppliers").select("*").order("name"); return (data ?? []).map(toSupplier); },
    getById:  async (id)     => { const { data } = await supabase.from("suppliers").select("*").eq("id", id).single(); return data ? toSupplier(data) : null; },
    add:      async (item)   => { const { data } = await supabase.from("suppliers").insert({ name: item.name, type: item.type, phone: item.phone ?? "", notes: item.notes ?? "" }).select().single(); return toSupplier(data); },
    update:   async (id, p)  => { const d = {}; if (p.name !== undefined) d.name = p.name; if (p.type !== undefined) d.type = p.type; if (p.phone !== undefined) d.phone = p.phone; if (p.notes !== undefined) d.notes = p.notes; await supabase.from("suppliers").update(d).eq("id", id); },
    delete:   async (id)     => { await supabase.from("suppliers").delete().eq("id", id); },
  },

  jobs: {
    getAll:   async ()       => { const { data } = await supabase.from("jobs").select("*").order("created_at", { ascending: false }); return (data ?? []).map(toJob); },
    getById:  async (id)     => { const { data } = await supabase.from("jobs").select("*").eq("id", id).single(); return data ? toJob(data) : null; },
    add:      async (item)   => { const { data } = await supabase.from("jobs").insert({ customer_id: item.customerId, car_id: item.carId, date_in: item.dateIn || null, date_out: item.dateOut || null, status: item.status, notes: item.notes ?? "", lines: item.lines ?? [], payments: item.payments ?? [] }).select().single(); return toJob(data); },
    update:   async (id, p)  => { const d = {}; if (p.customerId !== undefined) d.customer_id = p.customerId; if (p.carId !== undefined) d.car_id = p.carId; if (p.dateIn !== undefined) d.date_in = p.dateIn || null; if (p.dateOut !== undefined) d.date_out = p.dateOut || null; if (p.status !== undefined) d.status = p.status; if (p.notes !== undefined) d.notes = p.notes; if (p.lines !== undefined) d.lines = p.lines; if (p.payments !== undefined) d.payments = p.payments; await supabase.from("jobs").update(d).eq("id", id); },
    delete:   async (id)     => { await supabase.from("jobs").delete().eq("id", id); },
  },
};
